...
def train(args):
    ...
    # T5
        ...
        if args.method == 'prefix-tuning' or args.method == 'prefix-routing':
    ...
    for e in range(epoch):
        ...
        is_update =  is_better_score or ((val_score == max_val_score) and is_better_loss)
        ...
        if is_update:
            print("Updating result due to higher validation score or = validation score lower loss")
    ...
    # model.push_to_hub('AudreyTrungNguyen/hihi')
    # token = HfFolder.get_token()

    # # Create a repository on the Hugging Face Hub
    # api = HfApi()
    # repo = api.create_repo(repo_id='AudreyTrungNguyen/hihi', token=token)  # Replace 'username' with your actual username

    # # Add files to the repository and push them
    # repo.push_to_hub()
    l.close()
...
def prepare_dataset(dataset_name, tokenizer, batch_size, max_length, text_to_text, seed):
    ...
    train_dataloader = DataLoader(train_dataset, collate_fn=collate_fn, batch_size=batch_size, drop_last=False, shuffle=True, pin_memory=True, num_workers=4, worker_init_fn=lambda worker_id: seed_worker(args, worker_id), generator=g)
    val_dataloader = DataLoader(val_dataset, collate_fn=collate_fn, batch_size=batch_size, drop_last=False, shuffle=False, pin_memory=True, num_workers=4, worker_init_fn=lambda worker_id: seed_worker(args, worker_id), generator=g)   
    test_dataloader = DataLoader(test_dataset, collate_fn=collate_fn, batch_size=batch_size, drop_last=False, shuffle=False, pin_memory=True, num_workers=4, worker_init_fn=lambda worker_id: seed_worker(args, worker_id), generator=g)   
    return train_dataloader, val_dataloader, test_dataloader, num_labels, metric

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--seed', type=int)
    parser.add_argument('--subsetlr', type=float, default=2e-4)
    ...
    parser.add_argument('--num_shared_virtual_tokens', type=int, default=None)
    ...
    parser.add_argument('--shareType', type=str, default='Add')
    parser.add_argument('--sharedEmbeddinglr', type=float)
    parser.add_argument('--routerlr', type=float)
