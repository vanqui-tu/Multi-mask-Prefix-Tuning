...
from src_dept.tasks import AutoTask, num_labels_mapping
from src_dept.metrics import TASK_TO_METRICS
from src_dept.trainer import PEFTSeq2SeqTrainer
from src_dept.postprocessors import AutoPostProcessor, PostProcessor
...
class ModelArguments:
    ...
    num_virtual_tokens_full: int = field(
        default=None
    )
    routing_level: str = field(
        default="prompt"
    )
    perturb_router: bool = field(
        default=False
    )
    topk: int = field(
        default=1
...
def main():
    ...
    if any(x in model_args.model_name_or_path for x in ["bert", "roberta", "albert"]):
        logger.info(f"\n\nLoading encoder model from {model_args.model_name_or_path}.\n\n")
    ...
    if model_args.peft_model_id:
        ...
        elif model_args.peft_type == "prefix-tuning":
            peft_config = PrefixTuningConfig(task_type=task_type, num_virtual_tokens=model_args.num_virtual_tokens, apply_prefix_encoder_only=False) #, prefix_projection=True, encoder_hidden_size=512)
        elif model_args.peft_type == "p-tuning":
            peft_config = PromptEncoderConfig(task_type=task_type, num_virtual_tokens=model_args.num_virtual_tokens, encoder_hidden_size=128)
        elif model_args.peft_type == "prompt-tuning":
            peft_config = PromptTuningConfig(task_type=task_type, num_virtual_tokens=model_args.num_virtual_tokens)
        elif model_args.peft_type == "prompt-routing":
            peft_config = PromptRoutingConfig(task_type=task_type, num_virtual_tokens=model_args.num_virtual_tokens, num_virtual_tokens_full=model_args.num_virtual_tokens_full, perturb_router=model_args.perturb_router, topk=model_args.topk, stochastic=model_args.stochastic, gumbel=model_args.gumbel)
        elif model_args.peft_type == "prefix-routing":
            peft_config = PrefixRoutingConfig(task_type=task_type, num_virtual_tokens=model_args.num_virtual_tokens, num_virtual_tokens_full=model_args.num_virtual_tokens_full, perturb_router=model_args.perturb_router, topk=model_args.topk, stochastic=model_args.stochastic, gumbel=model_args.gumbel)
