...
class PrefixRoutingConfig(PromptLearningConfig):
    ...
    num_shared_virtual_tokens: Optional[int] = field(
        default=100, 
        metadata={
            "help": "The number of shared tokens"
        }
    )
    ...
    shareType: Optional[str] = field(
        default='Add', 
        metadata={
            "help": "Whether to Add or Prepend the shared prompt"
        }    
    )
    apply_adaptive_mask: Optional[bool] = field(
        default=False,
...
class PrefixRoutingEncoder(torch.nn.Module):
    def __init__(self, config, word_embeddings):
        super().__init__()
        self.config = config
        self.num_shared_virtual_tokens = config.num_shared_virtual_tokens
        self.embedding = torch.nn.Embedding(config.num_virtual_tokens_full, config.token_dim*config.num_layers * 2)

        self.sharedTokens = torch.arange(0, self.num_shared_virtual_tokens, device='cuda')
        self.sharedEmbedding = torch.nn.Embedding(self.num_shared_virtual_tokens, config.token_dim*config.num_layers * 2)
    ...
    def forward(self, indices, input_ids, inputs_embeds, attention_mask, base_model=None):
        ...
        if k == 1:
            idx = idx*num_virtual_tokens + torch.arange(0, num_virtual_tokens, device='cuda').unsqueeze(0)
        else:
            idx = (idx*num_virtual_tokens).unsqueeze(-1) + torch.arange(0, num_virtual_tokens, device='cuda').unsqueeze(0).unsqueeze(0)
        if not self.config.stochastic:
            # print(self.mask.shape)
            # print(self.subsetMask[idx_route].shape)
            ...
            # print(mask.shape)
            # print(subsetMask.shape)
                    mask = torch.cat((mask, subsetMask.squeeze(1)), dim=1)

            prompt_embeddings = self.embedding(idx) * values.unsqueeze(-1).unsqueeze(-1)
            tensor1 = torch.sum(prompt_embeddings, dim=1)
            # print(torch.sum(prompt_embeddings, dim=1).shape)
            if self.config.shareType == 'Prepend':
                # print(tensor1.shape)
                # print(self.sharedEmbedding(self.sharedTokens).repeat(batch_size, 1, 1).shape)
                prompt_embeddings = torch.cat((self.sharedEmbedding(self.sharedTokens).repeat(batch_size, 1, 1).squeeze(1),tensor1), dim = 1)
            
            elif self.config.shareType == 'Add':
                prompt_embeddings = self.sharedEmbedding(self.sharedTokens).repeat(batch_size, 1, 1).squeeze(1) + tensor1
            elif self.config.shareType == 'Only':
                prompt_embeddings = self.sharedEmbedding(self.sharedTokens).repeat(batch_size, 1, 1).squeeze(1)
            else:
                prompt_embeddings = tensor1
        ...
        if self.analysis:
            indices = torch.full((batch_size, 1), self.prompt_index * num_virtual_tokens, device='cuda').long() + torch.arange(0, num_virtual_tokens, device='cuda').repeat(batch_size, 1)
