...
class PromptRoutingConfig(PromptLearningConfig):
    ...
    shareType: Optional[str] = field(
        default='Add', 
        metadata={
            "help": "Whether to Add or Prepend"
        }    
    )
...
class PromptRoutingEmbedding(torch.nn.Module):
    def __init__(self, config, word_embeddings):
        ...
        self.embedding = torch.nn.Embedding(config.num_virtual_tokens_full, config.token_dim)

        self.sharedTokens = torch.arange(0, self.num_shared_virtual_tokens, device='cuda')
        self.sharedEmbedding = torch.nn.Embedding(self.num_shared_virtual_tokens, config.token_dim)

        # Linear router

        # if config.typeShared == 'Prepend':
        #     assert (config.num_virtual_tokens_full - self.num_shared_virtual_tokens) % (config.num_virtual_tokens- self.num_shared_virtual_tokens) == 0
        #     self.n_routes = (config.num_virtual_tokens_full - self.num_shared_virtual_tokens)// (config.num_virtual_tokens- self.num_shared_virtual_tokens)
    ...
    def forward(self, indices, input_ids, inputs_embeds, attention_mask, base_model=None):
        ...
        if not self.config.stochastic:
            ...
            tensor1 = torch.sum(prompt_embeddings, dim=1).squeeze()
            # expanded_tensor1 = tensor1.unsqueeze(1).expand(-1, 12, -1, -1).reshape(32, 12, 18432)
