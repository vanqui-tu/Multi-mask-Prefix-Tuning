...
from .tuners import LoraModel, PrefixEncoder, PromptEmbedding, PromptEncoder, PromptRoutingEmbedding, PrefixRoutingEncoder
from .utils import (
...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def _setup_prompt_encoder(self):
        ...
        elif self.peft_config.peft_type == PeftType.PREFIX_ROUTING:
            prompt_encoder = PrefixRoutingEncoder(self.peft_config, self.word_embeddings)
    ...
    def get_prefix_routing(self, batch_size, input_ids, inputs_embeds, attention_mask):
        peft_config = self.peft_config
        prompt_encoder = self.prompt_encoder
        ...
        #     past_key_values = prompt_encoder.embedding.weight.repeat(batch_size, 1, 1)
        # else:
        past_key_values, mask = prompt_encoder(prompt_tokens, input_ids, inputs_embeds, attention_mask)
        # print("shape past key", past_key_values.shape)
        if self.peft_config.shareType == 'Add':
            past_key_values = past_key_values.view(
                batch_size,
                peft_config.num_virtual_tokens,
                peft_config.num_layers * 2,
        ...
        elif self.peft_config.shareType == 'Only':
            past_key_values = past_key_values.view(
                batch_size,
                peft_config.num_shared_virtual_tokens,
                peft_config.num_layers * 2,
                peft_config.num_attention_heads,
                peft_config.token_dim // peft_config.num_attention_heads,
            )
            ...
            past_key_values = past_key_values.view(
                batch_size,
                peft_config.num_virtual_tokens + peft_config.num_shared_virtual_tokens,
                peft_config.num_layers * 2,
                peft_config.num_attention_heads,
                peft_config.token_dim // peft_config.num_attention_heads,
            )
        ...
        if peft_config.apply_adaptive_mask == True:
            ...
            past_key_values *= torch.sigmoid(mask).unsqueeze(3).unsqueeze(4).repeat(1,1, peft_config.num_transformer_submodules, peft_config.num_attention_heads, peft_config.token_dim // peft_config.num_attention_heads)
        ###
    ...
    **Attributes**:
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING or self.peft_config.peft_type == PeftType.PREFIX_ROUTING:
...
class PeftModelForCausalLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            past_key_values = self.get_prefix_routing(batch_size, input_ids, inputs_embeds, attention_mask)
            return self.base_model(input_ids=input_ids, past_key_values=past_key_values, **kwargs)
    ...
    def prepare_inputs_for_generation(self, *args, **kwargs):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            ...
            elif model_kwargs["past_key_values"] is None and self.peft_config.peft_type == PeftType.PREFIX_ROUTING:
                inputs_embeds = self.word_embeddings(model_kwargs["input_ids"])
                past_key_values = self.get_prefix_routing(model_kwargs["input_ids"].shape[0], model_kwargs["input_ids"], inputs_embeds, model_kwargs["attention_mask"])
                model_kwargs["past_key_values"] = past_key_values
...
class PeftModelForSeq2SeqLM(PeftModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if self.peft_config.peft_type == PeftType.PREFIX_ROUTING:
    ...
    def generate(self, **kwargs):
        ...
        self.base_model.prepare_inputs_for_generation = lambda *args, **kwargs: self.prepare_inputs_for_generation(input_ids, *args, **kwargs)
        self.base_model._prepare_encoder_decoder_kwargs_for_generation = (
            self._prepare_encoder_decoder_kwargs_for_generation
        )
        try:
            ...
            if self.peft_config.peft_type == PeftType.PREFIX_ROUTING:
                ...
                # past_prompt = self.get_prefix_routing(batch_size=batch_size, input_ids=input_ids, inputs_embeds=inputs_embeds, attention_mask=kwargs["attention_mask"])
                # kwargs["past_key_values"] = past_prompt
                return self.base_model.generate(**kwargs)
    ...
    def prepare_inputs_for_generation(self, input_ids, *args, **kwargs):
        peft_config = self.peft_config
        model_kwargs = self.base_model_prepare_inputs_for_generation(*args, **kwargs)
        if model_kwargs["past_key_values"] is None and (peft_config.peft_type == PeftType.PREFIX_TUNING or peft_config.peft_type == PeftType.PREFIX_ROUTING):
            ...
            if peft_config.peft_type == PeftType.PREFIX_TUNING:
                ...
                inputs_embeds = self.word_embeddings(input_ids)
                past_key_values = self.get_prefix_routing(input_ids.shape[0], input_ids, inputs_embeds, kwargs["attention_mask"])
    ...
    def get_past_prompt(self, batch_size):
        ...
        #     past_key_values = torch.cat([past_key_values, past_key_values], dim=2)
        # past_key_values = past_key_values.permute([2, 0, 3, 1, 4]).split(self.peft_config.num_transformer_submodules * 2)
